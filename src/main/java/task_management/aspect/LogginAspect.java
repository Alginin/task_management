...
public class LogginAspect {
    ...
    public void logBefore(JoinPoint joinPoint) {
        log.info("Запуск метода: {}", joinPoint.getSignature().toShortString());
    }

    @AfterThrowing(pointcut = "taskControllerMethods()", throwing = "exception")
    public void logAfterThrowing(JoinPoint joinPoint, Exception exception) {
        log.error("Ошибка в методе {}: {}", joinPoint.getSignature().toShortString(), exception.getMessage());
    }

    @AfterReturning(pointcut = "taskControllerMethods()", returning = "result")
    public void logAfterReturning(JoinPoint joinPoint, Object result) {
        log.info("Метод {} выполнен успешно. Результат: {}", joinPoint.getSignature().toShortString(), result);
    }
